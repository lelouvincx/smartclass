...
async function findWorkspaceExercise(c, exerciseId) {
...
async function findWorkspaceAssetSet(c, exerciseId, setId, columns = 'asset_set.*') {
  return c.env.DB.prepare(`
    select ${columns}
    from exercise_question_asset_sets asset_set
    join exercises exercise on exercise.id = asset_set.exercise_id
    where asset_set.id = ? and asset_set.exercise_id = ? and exercise.workspace_id = ?
      and exists (
        select 1
        from exercise_files source_file
        where source_file.id = asset_set.source_file_id
          and source_file.exercise_id = asset_set.exercise_id
      )
      and (
        asset_set.answer_source_file_id is null
        or exists (
          select 1
          from exercise_files answer_file
          where answer_file.id = asset_set.answer_source_file_id
            and answer_file.exercise_id = asset_set.exercise_id
        )
      )
  `).bind(setId, exerciseId, workspaceId(c)).first()
}
...
questionAssetsRoutes.post(
  ...
  async (c) => {
    ...
    const exercise = await findWorkspaceExercise(c, exerciseId)
    ...
    const sourceFile = await c.env.DB.prepare(`
      ...
      where file.id = ?
        and file.exercise_id = ?
        and file.file_type = 'exercise_pdf'
        and exercise.workspace_id = ?
    `).bind(source_file_id, exerciseId, workspaceId(c)).first()
    ...
    if (answer_source_file_id !== null) {
      const answerSourceFile = await c.env.DB.prepare(`
        ...
        where file.id = ?
          and file.exercise_id = ?
          and file.file_type = 'solution_pdf'
          and exercise.workspace_id = ?
      `).bind(answer_source_file_id, exerciseId, workspaceId(c)).first()
    ...
    try {
      const results = await c.env.DB.batch([
        ...
        c.env.DB.prepare(`
          with new_set as materialized (select last_insert_rowid() as id)
          ...
          select new_set.id, q_id, section_key, section_title,
            local_number, sub_id, type, correct_answer, max_score_hundredths
          from answer_schemas
          cross join new_set
          where exercise_id = ?
        `).bind(exerciseId),
        // Abort the whole batch if no schema rows were copied.
        c.env.DB.prepare(`
          insert into exercise_question_answer_schemas (
    ...
    const assetSet = await findWorkspaceAssetSet(c, exerciseId, setId)
...
questionAssetsRoutes.post(
  ...
  async (c) => {
    ...
    const assetSet = await findWorkspaceAssetSet(
      c,
      exerciseId,
      setId,
      'asset_set.id, asset_set.source_file_id, asset_set.answer_source_file_id, asset_set.confirmed_at',
    )
...
questionAssetsRoutes.post(
  ...
  async (c) => {
    ...
    const assetSet = await findWorkspaceAssetSet(
      ...
      exerciseId,
      setId,
      'asset_set.id, asset_set.confirmed_at',
    )
