...
- When Chinh says "PR merged", identify the PR's merge commit and confirm its `main` test and Worker deployment workflows passed, then poll `https://api.smartclass.lelouvincx.com/api/version` until `data.commit` equals the full merge commit hash. Then sync the local Git repository, switch back to the branch that was active before the bot PR work, close agent browser, clear the schedule, and archive the working thread. Do not clean up while any required check is pending or failed, or while production reports another commit.
