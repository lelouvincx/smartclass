...
### Take/Review 50/50 Two-Pane Layout (v0.4.5, PR #66)

- **Problem**: Old layout split content across 3 panels — `PdfSplitPane` (PDF 60% / content 40%) **inside** an outer 2-col grid `lg:grid-cols-[1fr_clamp(240px,_20rem,_40vw)]` with the answer-sheet sidebar in the right column. Effective PDF width ≈ 30% of viewport → illegible.
- **New layout**: 2-pane 50/50 on `lg+`. PDF on left, content on right. Right pane stacks vertically: sticky **answer-sheet card** (timer + nav grid + Submit/Exit) on top → input-mode card → question list. Same shape on `StudentReviewPage` (sticky `SubmissionReviewSidebar` on top of the answers table).
- **`PdfSplitPane` change**: outer flex `flex-[3]` / `flex-[2]` (60/40) → `lg:grid lg:grid-cols-2` (50/50). Hide-PDF toggle preserved. When PDF is hidden, the content pane fills the full width (no grid). `data-testid="pdf-pane"` and `data-testid="content-pane"` exposed for layout assertions.
- **Sidebar lives in-flow on all breakpoints**: dropped the mobile `Sheet` drawer, the floating "Answer Sheet" button, the floating mobile timer chip, the `sheetOpen` state, and the unused `Sheet*` / `LayoutGrid` imports from `StudentTakeExercisePage`. Sticky-top answer-sheet works for both desktop and mobile because the page is a single scrolling viewport — `lg:sticky lg:top-20` keeps it pinned during scroll past it; on mobile it just scrolls inline.
- **Rationale**: customer feedback after v0.4.5 — the PDF is the primary reference material; the answer-sheet just needs to stay reachable, not occupy a permanent column. One vertical scroll on mobile is simpler than the 3-place state (sidebar / floating chip / drawer) the previous design carried.
- **Tests**: `pdf-split-pane.test.jsx` gets layout assertions for the 50/50 grid and the full-width-when-hidden state. `StudentTakeExercisePage.test.jsx` drops the 3 mobile-chip regression tests (no longer applicable) and adds `'does not render a separate desktop sidebar column or mobile floating drawer'` (asserts a single Submit button) plus a DOM-order test that the timer precedes the first question card.

### Take Page Single-Question View (v0.4.5, PR #67)

- **Problem**: After PR #66, the right pane stacked the sticky answer-sheet card on top of the full vertical list of all 24 question cards. The sticky card visually overlapped the question list and the right pane was too long to scroll comfortably.
- **New behavior**: The right pane shows the answer-sheet sticky card and **exactly one question card** at a time. The student picks which question via the existing `QuestionNavGrid` cells (1–24) or via `← Previous` / `Next →` buttons in the question card footer. Default = first question (`questionGroups[0]`).
- **State**: Reuses the existing `currentQId` state (previously only used for the nav-grid highlight). `handleJump(qId)` now just sets `currentQId` — no `scrollIntoView`. `questionRefs` was deleted entirely (no scroll target needed).
- **Render**: `questionGroups.map(...)` was replaced with a single IIFE that finds the group by `currentQId` and renders one `Card`. Footer adds Previous/Next buttons + `Question N of M` status text.
- **Rationale**: customer feedback after #66 — packing the answer-sheet + 24 question cards into a half-width column made both feel cramped. One question at a time keeps the right pane focused and predictable; the nav grid is already a familiar UI for picking questions.
