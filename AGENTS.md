...
- Before changing product scope or milestone status, read the Roadmap in `README.md`.
- Before changing an established flow, read the relevant RFC under `docs/plans/` and any later RFC that supersedes it. RFCs preserve rationale; current routes, tests, migrations, and schema define current behavior.
...
- Publish every change through a pull request as `lelouvincx-bot`. If bot commit, push, or pull-request authentication is unavailable, stop rather than falling back to a personal identity.
