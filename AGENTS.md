...
- When Chinh says "PR merged", identify the PR's merge commit and confirm its `main` test and Worker deployment workflows passed, including the deployment's `/api/version` check that production reports the merge commit hash. Then sync the local Git repository, switch back to the branch that was active before the bot PR work, clear the schedule, and archive the working thread. Do not clean up while any required check is pending or failed.
